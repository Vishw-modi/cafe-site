    ...
    <section className="py-24 px-6 max-w-6xl mx-auto">
          ...
          {SPACE_ZONES.map((zone) => (
            <button
              ...
              className={`relative px-5 py-2.5 rounded-full text-xs uppercase tracking-wider font-medium transition-colors duration-300 ${
                activeSpace.id === zone.id ? "text-[#2C221E] dark:text-[#F5EFE6] font-semibold" : "text-[#2C221E]/60 dark:text-[#F5EFE6]/60 hover:text-[#2C221E] dark:hover:text-white"
              }`}
            >
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="grid md:grid-cols-2 gap-10 items-center bg-[#FDFBF7] dark:bg-[#1B1513] rounded-3xl p-6 sm:p-8 border border-[#E8E2D8] dark:border-[#2E2420] shadow-sm"
        >
          {/* Space Image */}
          <div className="relative h-[340px] sm:h-[400px] rounded-2xl overflow-hidden shadow-md" data-cursor="VIEW">
            <img
              src={activeSpace.image}
              alt={activeSpace.name}
              className="w-full h-full object-cover"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#2C221E]/30 dark:from-[#14100E]/50 via-transparent to-transparent" />
          </div>

          {/* Space Description */}
          <div className="space-y-6">
            <div>
              <span className="text-xs uppercase tracking-widest text-[#8F9E8B] dark:text-[#A1B39C] font-medium block mb-1">
                {activeSpace.subtitle}
              </span>
              <h3 className="font-serif text-3xl font-normal text-[#2C221E] dark:text-[#F5EFE6]">
                {activeSpace.name}
              </h3>
            </div>

            <p className="text-sm sm:text-base text-[#2C221E]/75 dark:text-[#F5EFE6]/75 leading-relaxed font-sans">
            ...
            <div className="grid grid-cols-2 gap-3 pt-2">
              {activeSpace.features.map((feat) => (
                <div key={feat} className="flex items-center gap-2 text-xs text-[#2C221E]/80 dark:text-[#F5EFE6]/80 font-medium">
                  <div className="p-1 rounded-full bg-[#8F9E8B]/15 dark:bg-[#A1B39C]/20 text-[#8F9E8B] dark:text-[#A1B39C]">
